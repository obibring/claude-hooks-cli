...
const { getFs } = _require("./get-fs.cjs")
...
export class HookHandler {
  ...
  /**
   * Creates a hook handler for the given event name or array of event names.
   * When an array is passed, the handler reads stdin and matches the
   * hook_event_name from the input to determine which event fired.
   *
   * @param {any} event
   * @param {any} [options]
   * @returns {any}
  ...
  static for(event, options) {
    if (Array.isArray(event)) {
      // Read stdin to determine which event fired, then create the handler
      const { readFileSync } = getFs()
      let raw
      try {
      ...
      const eventName = json.hook_event_name
      if (!event.includes(eventName)) {
        process.stderr.write(
          `Unexpected hook event: "${eventName}". Expected one of: ${event.join(", ")}\n`,
        )
        process.exit(2)
      }
      const handler = new HookHandler(/** @type {any} */ (eventName), options)
      // Pre-populate the cache since we already read stdin
      const result = handler._inputSchema.safeParse(json)
      if (!result.success) {
        process.stderr.write(
          `Input validation failed for ${eventName}: ${result.error.message}\n`,
        )
        process.exit(2)
      }
      handler._cachedInput = result.data
      return handler
    }
  ...
  parseInput() {
    ...
    let raw
    try {
      raw = getFs().readFileSync(0, "utf-8")
    } catch (/** @type {any} */ e) {
      process.stderr.write(`Failed to read stdin: ${e.message}\n`)
      process.exit(2)
    }
  ...
  getEnvFileVars(options) {
    ...
    try {
      if (this._options?.readFile) {
        ...
        raw = getFs().readFileSync(filePath, "utf-8")
      }
    } catch {
      this._cachedEnvFileVars = {}
      return this._cachedEnvFileVars
  ...
  writeEnvFile(vars) {
    ...
    if (this._options?.writeFile) {
      this._options.writeFile(filePath, contents, { encoding: "utf-8" })
    } else {
      getFs().writeFileSync(filePath, contents, "utf-8")
    }
   ...
   * Only available for SessionStart, CwdChanged, and FileChanged hooks.
   * Uses the custom `writeFile` function from constructor options if provided
   * (reads existing content first, then writes the full content with the new var appended),
   ...
   */
  appendToEnvFile(variableName, value) {
    const filePath = process.env.CLAUDE_ENV_FILE
    if (!filePath) return
    ...
    if (this._options?.writeFile) {
      ...
      try {
        if (this._options.readFile) {
          existing = this._options.readFile(filePath)
        } else {
          existing = getFs().readFileSync(filePath, "utf-8")
        }
      } catch {
        // File doesn't exist yet — start fresh
      }
      this._options.writeFile(filePath, existing + line, { encoding: "utf-8" })
    } else {
      getFs().appendFileSync(filePath, line, "utf-8")
    }
