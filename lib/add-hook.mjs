...
import { resolve, dirname, extname, join, parse } from "node:path"
 ...
 */
/**
 * Determines if a .js file should use ESM import or CJS require
 * by finding the nearest package.json and checking its "type" field.
 *
 * @param {string} filePath - Absolute path to the .js file
 * @returns {boolean} true if the package uses ESM ("type": "module")
 */
...
/**
 * Determines the module style for a file based on its extension.
 *
 * @param {string} filePath - Absolute path to the file
 * @param {"ts" | "js"} lang - Language chosen by user
 * @returns {"esm" | "cjs"}
 */
function getModuleStyle(filePath, lang) {
  if (lang === "ts") return "esm"
  const ext = extname(filePath).toLowerCase()
  if (ext === ".mjs") return "esm"
  if (ext === ".cjs") return "cjs"
  // .js — check package.json
  return isPackageEsm(filePath) ? "esm" : "cjs"
}

/**
 * Generates a hook script template with the correct import style.
 *
 * @param {string} eventName - The hook event name
 * @param {"ts" | "js"} lang - Language variant
 * @param {"esm" | "cjs"} moduleStyle - Import style
 * @returns {string}
 */
function generateHookScript(eventName, lang, moduleStyle) {
  const isToolEvent = [
    "PreToolUse",
    "PostToolUse",
    "PostToolUseFailure",
 ...
 *   //            WebFetch, WebSearch, Agent, AskUserQuestion`
  ...
  const shebang =
    lang === "ts" ? "#!/usr/bin/env npx tsx" : "#!/usr/bin/env node"
...
const handler = HookHandler.for("${eventName}")
...
async function getCommandInteractive(eventName) {
  ...
  if (shouldWrite) {
    ...
    const moduleStyle = getModuleStyle(resolved, choice)
    const template = generateHookScript(eventName, choice, moduleStyle)
