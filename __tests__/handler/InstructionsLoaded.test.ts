...
describe(`HookHandler("${EVENT}")`, () => {
  it("parses valid input and returns all fields", async () => {
    ...
    expect(parsed.file_path).toBe("/test/CLAUDE.md")
    expect(parsed.memory_type).toBe("claude_md")
    expect(parsed.load_reason).toBe("session_start")
  })
  ...
  it("emits valid output that passes output schema", async () => {
    ...
    const parsed = InstructionsLoadedOutputSchema.safeParse(emitted)
    expect(parsed.success).toBe(true)
    expect(emitted.additionalContext).toBe("loaded")
  })
